...
internal static class LayoutRules
{
        ...
        string current)
    {
        ...
        if (IsTopLevelImport(previous) && IsTopLevelImport(current))
            return false;
    ...
    {
        return IsTopLevelImport(line) || IsTopLevelMajorDeclaration(line);
    }

    private static bool IsTopLevelMajorDeclaration(string line)
    {
        return StartsWithKeyword(line, "fn")
            || StartsWithKeyword(line, "struct")
            || StartsWithKeyword(line, "enum")
            || StartsWithKeyword(line, "impl");
    }

    private static bool IsTopLevelImport(string line)
    {
        return StartsWithKeyword(line, "import");
    }
